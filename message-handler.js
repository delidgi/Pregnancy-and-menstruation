...
import { scanMessage, scanDateTag, scanStatusTag, scanWeeksFromText, scanPregnancyStateTags, scanChildUpdateTags, stripHiddenTags, stripThink } from './scanner.js';
import { applyScanResult, createPregnancyFromWeeks, createPregnancyFromStateTag } from './pregnancy.js';
import { checkPartnerConception, applyPartnerBirth, createPartnerPregnancyFromStateTag, applyPartnerPregnancyStateTag, advancePartnerTime, getPartnerName } from './partner.js';
import { isOmegaverse, ensureOmegaFields, advanceOmegaCycle, getCfg } from './omegaverse.js';
...
function runScan() {
    ...
    if (tagResult) {
        ...
        // ── Партнёрские теги (этап 2) — обрабатываем ДО юзерского return-блока:
        //    оба события могут прийти в одном сообщении (например, оба зачали)
        if (p.partner?.enabled) {
            const pp = p.partner;

            // Пол плода партнёра раскрыт
            if (tagResult.partner_sex_revealed && pp.isPregnant && !pp.fetusSexRevealed) {
                if (tagResult.revealed_sexes && tagResult.revealed_sexes.length > 0) {
                    const need = pp.fetusCount || 1;
                    const newSex = [];
                    for (let i = 0; i < need; i++) {
                        newSex.push(tagResult.revealed_sexes[i] || tagResult.revealed_sexes[tagResult.revealed_sexes.length - 1]);
                    }
                ...
                dlog(`[Reproductive] Partner fetus sex REVEALED: ${pp.fetusSex.join(', ')}`);
                saveSettingsDebounced();
                if (s.showNotifications) {
                    const icons = pp.fetusSex.map(sx => sx === 'M' ? '♂ мальчик' : '♀ девочка').join(', ');
                    showNotification(`<i class="fa-solid fa-baby"></i> ${getPartnerName(p)} — пол определён: ${icons}`, 'success');
                }
                syncUI();
                updatePromptInjection();
                setTimeout(renderInfoblock, 500);
            ...
            // Зачатие партнёра (с анти-дабл-роллом и защитой после сброса)
            if (tagResult.partner_conception && !pp.isPregnant) {
                if (s._lastPartnerConceptionRollAt === positionId) {
                    dlog('[Reproductive] Partner conception already rolled at this position — skipping');
                } else if (s._partnerConceptionBlockedUntil && positionId <= s._partnerConceptionBlockedUntil) {
                    dlog(`[Reproductive] Partner conception blocked (reset protection until pos ${s._partnerConceptionBlockedUntil})`);
                } else {
                    s._lastPartnerConceptionRollAt = positionId;
                    checkPartnerConception();
                    pushStateHistory(positionId);
                }
            }

            // Роды партнёра
            if (tagResult.partner_birth && pp.isPregnant) {
                if (applyPartnerBirth(tagResult)) {
                    _preRegenSnapshot = snapshotOf(getPregnancyData());
                    _snapshotChatId = chatIdNow;
                    pushStateHistory(positionId);
                    if (s.showNotifications) {
                        showNotification(`<i class="fa-solid fa-baby"></i> ${getPartnerName(p)}: роды!`, 'success');
                    }
                }
            }
        }
    ...
    // 1.55) CHILD_UPDATE — модель сама дополняет профили детей (черты/внешность/лор).
    // Мердж бережный (applyChildUpdate): ничего не перезаписывает, только дополняет.
    const childUpdates = scanChildUpdateTags(text);
    if (childUpdates) {
        import('./family.js').then(fam => {
            let any = false;
            for (const upd of childUpdates) {
                const res = fam.applyChildUpdate(p, upd);
                if (res) {
                    any = true;
                    const nm = res.child.name || 'ребёнок';
                    fam.logChronicle(p, 'fa-wand-magic-sparkles', `${nm}: ${res.changes.join(', ')}`);
                    if (s.showNotifications) {
                        showNotification(`<i class="fa-solid fa-wand-magic-sparkles"></i> Профиль ${nm}: ${res.changes.join(', ')}`, 'info');
                    }
                }
            }
            if (any) {
        ...
        }).catch(e => dwarn('[Reproductive] CHILD_UPDATE apply failed:', e));
    }
    ...
    for (const pregState of pregStates) {

    // Партнёрский тег (carrier:"partner") → свой роутинг; юзерская логика ниже не трогается
    if (pregState.carrier === 'partner') {
        if (!p.partner?.enabled) {
            dlog('[Reproductive] PREGNANCY_STATE carrier=partner ignored — partner tracking disabled');
        } else if (!p.partner.isPregnant) {
            const blockedP = s._partnerConceptionBlockedUntil && positionId <= s._partnerConceptionBlockedUntil;
            if (!blockedP) createPartnerPregnancyFromStateTag(pregState);
        } else {
            applyPartnerPregnancyStateTag(pregState);
        }
        continue;
    }
    ...
    } // конец цикла по PREGNANCY_STATE тегам
...
export function rescanMessage(fullText, messageIndex) {
    ...
    if (tagResult) {
        ...
        // ── Партнёрские теги при rescan (этап 2) ──
        if (p.partner?.enabled) {
            const pp = p.partner;
            if (tagResult.partner_sex_revealed && pp.isPregnant && !pp.fetusSexRevealed) {
                if (tagResult.revealed_sexes && tagResult.revealed_sexes.length > 0) {
                ...
                pp.fetusSexRevealed = true;
                dlog(`[Reproductive] (rescan) Partner fetus sex REVEALED: ${pp.fetusSex.join(', ')}`);
                saveSettingsDebounced();
                syncUI();
                updatePromptInjection();
                setTimeout(renderInfoblock, 300);
            ...
            if (tagResult.partner_conception && !pp.isPregnant) {
                if (s._lastPartnerConceptionRollAt === positionId) {
                    dlog('[Reproductive] (rescan) Partner conception already rolled at this position — skipping');
                } else if (s._partnerConceptionBlockedUntil && positionId <= s._partnerConceptionBlockedUntil) {
                    dlog('[Reproductive] (rescan) Partner conception blocked (reset protection)');
                } else {
                    s._lastPartnerConceptionRollAt = positionId;
                    checkPartnerConception();
                    pushStateHistory(positionId);
...
function applyStatusData(s, p, data) {
    ...
    // ── Партнёрский под-объект "partner":{...} в RP_STATUS (этап 2) ──
    if (data.partner && typeof data.partner === 'object' && p.partner?.enabled) {
        const pp = p.partner;
        const dp = data.partner;
        if (dp.mood) pp.mood = dp.mood;
        if (dp.libido) pp.libido = dp.libido;
        if (pp.isPregnant) {
            if (dp.weight_gain) pp.weightGain = dp.weight_gain;
            if (dp.baby_activity) pp.babyActivity = dp.baby_activity;
        }
        pp._dynamic = {
            note: dp.note || null,
            symptoms: dp.symptoms || null,
        };
    }
}
...
function advanceTime(s, p, daysPassed) {
    ...
    // ── Партнёр: цикл, недели беременности, авто-роды (этап 2) ──
    advancePartnerTime(s, p, daysPassed);
    ...
    // ── Семейные события: стадии жизни и дни рождения (активные + выросшие дети) ──
    if (p.rpDate && ((p.babies && p.babies.length > 0) || (p.grownChildren && p.grownChildren.length > 0))) {
        maybeProcessFamilyEvents(s, p, daysPassed);
    }
...
// ─── Семейные события: инициализация стадий, дни рождения, диалог взросления ───
let _stageDialogShowing = false;
function maybeProcessFamilyEvents(s, p, daysPassed) {
    try {
        // Динамические импорты — тот же паттерн, что у maybeGraduateBabies (избегаем циклов)
        import('./family.js').then(fam => {
            // 1) Дети из старых сохранений без поля stage — тихо ставим вычисленную стадию,
            //    иначе первый же RP_DATE обрушит каскад диалогов за все пропущенные стадии.
            if (fam.ensureStages(p)) saveSettingsDebounced();

            // 2) Дни рождения, пересечённые прошедшим RP-временем
            const bdays = fam.collectBirthdays(p, daysPassed);
            if (bdays.length > 0) {
                for (const b of bdays) {
                    fam.logChronicle(p, 'fa-cake-candles', `${b.name} — ${b.years} ${fam.ruYears(b.years)}!`);
                    if (s.showNotifications) {
                        showNotification(`<i class="fa-solid fa-cake-candles"></i> День рождения: ${b.name} — ${b.years} ${fam.ruYears(b.years)}!`, 'success');
                    }
                }
                saveSettingsDebounced();
                updatePromptInjection();
            ...
            // 2.5) Случайные события (Sims-style): истечение старых + шанс новых
            const chancePerDay = Math.max(0, Math.min(50, parseInt(s.familyEventChance ?? 15) || 0)) / 100;
            const evRes = fam.processFamilyEvents(p, daysPassed, chancePerDay);
            if (evRes.started.length > 0 || evRes.ended.length > 0) {
                for (const { child, event } of evRes.started) {
                    const nm = child.name || 'Малыш';
                    fam.logChronicle(p, 'fa-dice', `${nm}: ${event.ru}`);
                    if (s.showNotifications) {
                        showNotification(`<i class="fa-solid fa-dice"></i> ${nm}: ${event.ru}`, 'info');
                    }
                }
                saveSettingsDebounced();
                updatePromptInjection();
            ...
            // 2.7) Омегаверс: презентация вторичного пола (A/B/O) у подростков.
            // Идемпотентно: срабатывает один раз на ребёнка (child.designation остаётся).
            if (p.universe === 'omegaverse') {
                import('./omegaverse.js').then(om => {
                    let presented = false;
                    for (const { child } of fam.getAllChildren(p)) {
                        if (child.designation) continue;
                        const stage = fam.getStageOf(child, p.rpDate);
                        if (!stage || fam.stageIndex(stage.id) < fam.stageIndex('teen')) continue;
                        const parents = om.inferParentDesignations(p, child);
                        child.designation = om.rollChildDesignation(parents.father, parents.mother);
                        const label = om.DESIGNATION_LABELS[child.designation]?.ru || child.designation;
                        const nm = child.name || 'Ребёнок';
                        fam.logChronicle(p, 'fa-star-of-life', `${nm} презентовался: ${label}`);
                        if (s.showNotifications) {
                            showNotification(`🌟 ${nm} презентовал${child.sex === 'F' ? 'ась' : 'ся'} как ${label.toUpperCase()}!`, 'success');
                        }
                        presented = true;
                    }
                    if (presented) {
                ...
                }).catch(e => dwarn('[Reproductive] presentation check failed:', e));
            }

            // 3) Переходы стадий — диалог с выбором новой черты характера
            const ups = fam.collectStageUps(p);
            if (ups.length === 0 || _stageDialogShowing) return;
            _stageDialogShowing = true;
            import('./family-ui.js').then(fui => {
                fui.showStageUpDialog(ups, (choices) => {
                    fam.applyStageUps(p, ups, choices);
                ...
                dwarn('[Reproductive] stage-up dialog import failed:', e);
                // Тихий fallback: стадии всё равно обновляем, черты не добавляем
                fam.applyStageUps(p, ups, []);
                _stageDialogShowing = false;
                saveSettingsDebounced();
            });
        }).catch(e => {
            dwarn('[Reproductive] family events check failed:', e);
        });
    } catch (e) {
        dwarn('[Reproductive] maybeProcessFamilyEvents error:', e);
    }
}
...
export function processDateTag(text) {
    ...
    // ── Ручные малыши, добавленные ДО первой RP-даты ──
    // Их birthRpDate временно якорился к реальному времени — перепривязываем
    // к RP-оси, сохраняя возраст, который задал юзер при добавлении.
    if (Array.isArray(p.babies)) {
        for (const baby of p.babies) {
            if (baby._birthRealAnchored) {
                const ageDays = Math.max(0, parseInt(baby._manualAgeDays) || 0);
                baby.birthRpDate = new Date(rpDate.getTime() - ageDays * 86400000).toISOString();
                delete baby._birthRealAnchored;
                delete baby._manualAgeDays;
                dlog(`[Reproductive] Manual baby "${baby.name || 'unnamed'}" re-anchored to RP timeline (${ageDays}d old)`);
            }
        }
    }
    ...
    if (prevRaw) {
        ...
        // Потолок 100 лет — только защита от мусорных дат (перепутан век и т.п.).
        // Раньше стояло 365, и любой таймскип больше года молча замораживал
        // возраст/стадии/дни рождения детей — династии живут годами!
