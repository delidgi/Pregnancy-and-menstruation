    ...
    try {
        ...
        const processFinalMessage = (messageIndex) => {
            try {
                ...
                if (
                    msg.mes.includes('<!--') &&
                    // Префиксные проверки без ']' — ловят и юзерские теги, и партнёрские
                    // варианты с суффиксом :PARTNER (этап 2)
        ...
        if (event_types.GENERATION_STARTED) {
            ...
            eventSource.on(event_types.GENERATION_STARTED, (genType, params, dryRun) => {
                ...
                if (genType === 'regenerate' || genType === 'swipe') {
                    dlog('[Reproductive] Regeneration detected via GENERATION_STARTED');
                    markRegeneration();
                }
            });
        }
